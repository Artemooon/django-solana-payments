...
from django_solana_payments.choices import SolanaPaymentStatusTypes, OneTimeWalletStateTypes, TokenTypes
from django_solana_payments.exceptions import PaymentNotFoundError, PaymentExpiredError, \
    PaymentTokenPriceNotFoundError, InvalidPaymentAmountError, PaymentNotConfirmedError
from django_solana_payments.helpers import get_solana_payment_model, get_solana_payment_related_name
from django_solana_payments.models import OneTimePaymentWallet, SolanaPayPaymentCryptoPrice, \
    AbstractPaymentToken
from django_solana_payments.services.main_wallet_service import send_solana_transaction_to_main_wallet
...
from django_solana_payments.solana.solana_transaction_query_client import SolanaTransactionQueryClient
...
class VerifyTransactionService:
    def __init__(self):
        self.solana_balance_client = SolanaBalanceClient(base_solana_client=base_solana_client)
        self.solana_token_client = SolanaTokenClient(base_solana_client=base_solana_client)
        self.solana_transaction_query_client = SolanaTransactionQueryClient(base_solana_client=base_solana_client)

    def verify_transaction_and_process_payment(
        self, payment_address: str,
            payment_crypto_token: Type[AbstractPaymentToken],
            meta_data: dict[str, Any] = None,
            send_payment_accepted_signal: bool = True,
            on_success: callable = None,
    ) -> SolanaPaymentStatusTypes:
        logger.info(f"Starting verification for payment_address={payment_address}, and token: {payment_crypto_token.mint_address}")

        solana_payment = SolanaPayment.objects.select_related("one_time_payment_wallet").filter(payment_address=payment_address).first()

        if not solana_payment:
            raise PaymentNotFoundError(payment_address)

        OneTimePaymentWallet.objects.filter(id=solana_payment.one_time_payment_wallet.id).update(
        ...
        recipient_wallet_transactions, payment_balance = self.validate_transfer_amount(
            solana_payment, receiver_address, payment_crypto_token, payment_crypto_token.token_type
        )
        logger.info(f"Found {len(recipient_wallet_transactions)} recipient transactions, balance={payment_balance}")
        ...
        if recipient_wallet_transactions:
            ...
            transaction_status = self.accept_verified_transaction_and_process_payment(
                paid_transaction, payment_balance, payment_crypto_token, solana_payment, meta_data
            )
            ...
            logger.warning(f"No recipient transactions found for payment_address={payment_address}")
    ...
    def _is_transaction_confirmed(self, transaction: GetTransactionResp) -> bool:
        ...
        transaction_statuses = self.solana_transaction_query_client.get_signatures_statuses([transaction_sig])
        ...
        if user_commitment == Finalized:
            ...
            return confirmation_status in (TransactionConfirmationStatus.Confirmed, TransactionConfirmationStatus.Finalized)
        elif user_commitment == Processed:
            return confirmation_status in (TransactionConfirmationStatus.Processed, TransactionConfirmationStatus.Confirmed, TransactionConfirmationStatus.Finalized)
        return False

    def accept_verified_transaction_and_process_payment(self, paid_transaction: GetTransactionResp,
        ...
        paid_transaction_signatures = paid_transaction.value.transaction.transaction.signatures

        paid_transaction_signature = paid_transaction_signatures[len(paid_transaction_signatures) - 1]

        transaction_status = self.commitment_to_payment_status(solana_payments_settings.PAYMENT_ACCEPTANCE_COMMITMENT)
        ...
        if send_funds_to_main_wallet_immediately:
            send_solana_transaction_to_main_wallet(
                ...
                payment_balance,
                TransactionTypeEnum.SPL.value if payment_crypto_token.token_type == TokenTypes.SPL else TransactionTypeEnum.NATIVE.value,
    ...
    @staticmethod
    def commitment_to_payment_status(commitment: Commitment) -> SolanaPaymentStatusTypes:
        """
    ...
    def validate_solana_payment(self, solana_payment: SolanaPayment) -> SolanaPaymentStatusTypes | None:
        if not solana_payment:
            raise PaymentNotFoundError(solana_payment.payment_address)

        if solana_payment.status in [SolanaPaymentStatusTypes.CONFIRMED, SolanaPaymentStatusTypes.FINALIZED]:
            logger.info(f"Payment already confirmed/finalized: status={solana_payment.status}")
        ...
        if timezone.now() > solana_payment.expiration_date:
            ...
            logger.warning(f"Payment expired: payment_address={solana_payment.payment_address}")
            raise PaymentExpiredError()

        return None

    def validate_transfer_amount(
        self, solana_payment: SolanaPayment, receiver_address: Pubkey,
            payment_crypto_token: Type[AbstractPaymentToken],
        ...
        if token_type == TokenTypes.SPL:
            ...
            target_address = self.solana_token_client.get_or_create_associated_token_address(
                receiver_address, Pubkey.from_string(payment_crypto_token.mint_address)
            )
        else:
            balance = self.solana_balance_client.get_balance_by_address(receiver_address)
        ...
        all_transactions = self.solana_transaction_query_client.get_transactions_for_address(address=target_address)
        # Exclude transactions sent by the configured sender
        recipient_wallet_transactions: list[GetTransactionResp] = []
        for tx in all_transactions:
            fee_payer = self.solana_transaction_query_client.extract_fee_payer_from_transaction_details(tx)
            if fee_payer:
                if fee_payer == Pubkey.from_string(solana_payments_settings.SOLANA_SENDER_ADDRESS):
                    logger.warning(
        ...
        if recipient_wallet_transactions and expected_amount > balance:
            logger.error(f"Invalid transfer amount: expected={expected_amount}, actual={balance}")
