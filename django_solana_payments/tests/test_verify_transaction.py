...
from django_solana_payments.services.verify_transaction_service import VerifyTransactionService
from django_solana_payments.helpers import get_solana_payment_model, get_payment_crypto_token_model
...
class TestVerifyTransactionAndProcessPayment:
    ...
    def test_payment_expired(self, expired_payment, payment_token):
        ...
        assert expired_payment.one_time_payment_wallet.state == OneTimeWalletStateTypes.PAYMENT_EXPIRED
    ...
    @pytest.mark.django_db
    @patch('django_solana_payments.services.verify_transaction_service.SolanaBalanceClient')
    @patch('django_solana_payments.services.verify_transaction_service.SolanaTransactionQueryClient')
    def test_no_transactions_found(self, mock_query_client_class, mock_balance_client_class,
                                   solana_payment, payment_token, payment_crypto_price):
    ...
    @pytest.mark.django_db
    @patch('django_solana_payments.services.verify_transaction_service.send_solana_transaction_to_main_wallet')
    @patch('django_solana_payments.services.verify_transaction_service.SolanaBalanceClient')
    @patch('django_solana_payments.services.verify_transaction_service.SolanaTransactionQueryClient')
    def test_successful_payment_verification_native_sol(
        self, mock_query_client_class, mock_balance_client_class, mock_send_to_main,
        solana_payment, payment_token, payment_crypto_price, test_settings, settings
    ):
        ...
        mock_query_client.extract_fee_payer_from_transaction_details.return_value = mock_fee_payer
        ...
        # Verify result
        assert result in [SolanaPaymentStatusTypes.CONFIRMED, SolanaPaymentStatusTypes.FINALIZED]

        # Verify payment was updated
        solana_payment.refresh_from_db()
        assert solana_payment.status in [SolanaPaymentStatusTypes.CONFIRMED, SolanaPaymentStatusTypes.FINALIZED]
    ...
    def test_invalid_payment_amount(
        self, mock_query_client_class, mock_balance_client_class,
        solana_payment, payment_token, payment_crypto_price, test_settings, settings
    ):
        ...
        mock_balance_client = MagicMock()
        mock_balance_client.get_balance_by_address.return_value = Decimal("0.05")  # Less than expected 0.1
        mock_balance_client_class.return_value = mock_balance_client
    ...
    def test_wallet_state_updated_to_processing(self, solana_payment, payment_token):
        ...
        with patch.object(service, 'validate_solana_payment', return_value=SolanaPaymentStatusTypes.CONFIRMED):
            service.verify_transaction_and_process_payment(
                payment_address=solana_payment.payment_address,
                payment_crypto_token=payment_token
            )
        ...
        assert solana_payment.one_time_payment_wallet.state == OneTimeWalletStateTypes.PROCESSING_PAYMENT

    @pytest.mark.django_db
    @patch('django_solana_payments.services.verify_transaction_service.solana_payment_accepted')
    @patch('django_solana_payments.services.verify_transaction_service.send_solana_transaction_to_main_wallet')
    @patch('django_solana_payments.services.verify_transaction_service.SolanaBalanceClient')
    @patch('django_solana_payments.services.verify_transaction_service.SolanaTransactionQueryClient')
    def test_signal_sent_on_success(
        self, mock_query_client_class, mock_balance_client_class, mock_send_to_main, mock_signal,
        solana_payment, payment_token, payment_crypto_price, test_settings, settings
    ):
        ...
        mock_query_client = MagicMock()
        mock_transaction = MagicMock(spec=GetTransactionResp)
        mock_transaction.value.transaction.transaction.signatures = [Signature.from_string("5" * 88)]

        mock_tx_status = MagicMock()
        mock_tx_status.confirmation_status = TransactionConfirmationStatus.Confirmed
        mock_query_client.get_signatures_statuses.return_value = [mock_tx_status]
        mock_query_client.get_transactions_for_address.return_value = [mock_transaction]
        mock_query_client.extract_fee_payer_from_transaction_details.return_value = Pubkey.from_string("2" * 44)
        mock_query_client_class.return_value = mock_query_client
        ...
        assert call_kwargs['transaction_status'] in [SolanaPaymentStatusTypes.CONFIRMED, SolanaPaymentStatusTypes.FINALIZED]
    ...
    def test_on_success_callback_called(
        ...
        assert args[1] in [SolanaPaymentStatusTypes.CONFIRMED, SolanaPaymentStatusTypes.FINALIZED]
